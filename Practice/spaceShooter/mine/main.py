import sys
sys.path.append("Practice/spaceShooter/mine/game/")

import cv2
import spaceShooter as game
import pdb
...
class BrainDQNMain(object):
    ...
    def setPerception(self, nextObservation, action, reward, terminal):
        ...
        newState = np.append(   #append给currentState添加数据
            self.currentState[1:, :, :], nextObservation, axis=0)   #沿行方向添加数据
    ...
    def setInitState(self, observation):
        ...
        print(self.currentState.shape)  #shape(4,80,80) 80*80是图片大小，4是图片个数
...
if __name__ == '__main__':
    ...
    ret, observation0 = cv2.threshold(observation0, 1, 255, cv2.THRESH_BINARY)  #返回值observation是目标图像
