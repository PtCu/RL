...
def main_menu():
    ...
    while True:
        ...
        if ev.type == pygame.KEYDOWN:
            ...
            draw_text(screen, "Press [ENTER] To Begin",
                      30, WIDTH / 2, HEIGHT / 2)
            draw_text(screen, "or [Q] To Quit", 30,
                      WIDTH / 2, (HEIGHT / 2) + 40)
            pygame.display.update()
...
for i in range(len(meteor_list)):
    _global = meteor_list[i]
    print("img_name:" + _global)
    meteor_images.append(pygame.image.load(
        path.join(img_dir, meteor_list[i])).convert())
