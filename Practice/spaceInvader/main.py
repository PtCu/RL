...
env = wrap_deepmind(env_raw, frame_stack=False,
...
policy_net.apply(policy_net.init_weights)  # 将weight_init应用在子模块上
...
sa = m.ActionSelector(EPS_START, EPS_END, policy_net,
                      EPS_DECAY, n_actions, device)
...
def evaluate(step, policy_net, device, env, n_actions, eps=0.05, num_episode=5):
    ...
    f = open("file.txt", 'a')
    f.write("%f, %d, %d\n" % (float(sum(e_rewards)) /
                              float(num_episode), step, num_episode))
    f.close()
...
for step in progressive:
    ...
    #     evaluate(step, policy_net, device, env_raw,
    #              n_actions, eps=0.05, num_episode=15)
